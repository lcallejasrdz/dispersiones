...
<div class="row">
    ...
    <div class="col-md-4"><h6 class="mb-0 text-uppercase">Monto: ${{ number_format($movement->amount_entry, 2) }}</h6></div>
</div>
<hr/>
...
<table class="table mb-0 table-striped table-bordered" id="entry-table">
    ...
    <tfoot>
        <tr>
            <th></th>
            <th class="text-end"><span>${{ number_format($movement->amount_entry, 2) }}</span></th>
            <th></th>
            <th></th>
            <th></th>
            <th></th>
...
@else
    <form id="form-create" class="row g-3 needs-validation" method="POST" action="{{ route('following.payroll_following_store', ['id' => $movement->id]) }}">
        <input type="hidden" name="_token" value="{{ csrf_token() }}">
        <input type="hidden" id="entry_total" value="{{ $movement->amount_entry }}">
...
@section("script")
    ...
    <script>
        ...
        $('#submit-btn').click(function(){
            ...
            if((dispersion_total == 0 || dispersion_total == '' || dispersion_total == null) || (dispersion_total != entry_total))
            {
                var alert = '<div class="alert alert-danger border-0 bg-danger alert-dismissible fade show py-2">';
                    alert += '<div class="d-flex align-items-center">';
                        alert += '<div class="font-35 text-white"><i class="bx bxs-message-square-x"></i>';
                        ...
                        alert += '<div class="ms-3">';
                            ...
                            alert += '<div class="text-white">';
                                alert += '<ul>';
                                    if(dispersion_total != entry_total){
                                        alert += '<li>El monto total de dispersiones no puede ser diferente al monto total del movimiento: D:'+ dispersion_total +' O:'+ entry_total +'</li>';
                                    }else if(dispersion_total == 0 || dispersion_total == '' || dispersion_total == null){
