...
<hr>
<form id="form-create" class="row g-3 needs-validation" method="POST" action="{{ route('invoice.accounting_store') }}" enctype="multipart/form-data">
                ...
                @foreach($direct_movements as $item)
                    <tr>
                        ...
                        @if($item->receipt == '' || $item->receipt == null)
                            <td class="text-center">
                                <input type="hidden" name="movement_entry_id[]" value="{{ $item->id }}" />
                                <input class="form-control form-control-sm" name="receipt[]" type="file" accept=".jpg,.jpeg,.png,.pdf">
                            </td>
                        @else
                            <td class="text-center">
                                <a href="{{ url('/uploads/accounting_invoices/'.$item->receipt) }}" target="_blank">
...
@section("script")
    ...
    <script>
        ...
        $('#date-btn').click(function(){
            ...
            {
                document.location.href="/invoice/accounting/"+year+"/"+month;
            }
        });
